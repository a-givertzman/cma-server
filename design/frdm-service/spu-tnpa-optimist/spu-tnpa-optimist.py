...
def rope_parameters(X1, Y1, X2, Y2, D1, D2, k, j, block_pair=None):
    ...
    # Сохраняем исходный направленный угол каната. Он нужен для
    # проверки перекидывания: например, 190° и -170° задают одно
    # направление, но сравнение с порогом 90° для них различается.
    # Нормализуется не сам alpha_rope, а только разность соседних
    # углов при расчете угла обхвата блока.
...
if __name__ == "__main__":
    ...
    for idx, block in enumerate(blocks):
        match block.bind:
            case BlockBindFixed():
                # Неподвижные блоки задаются в локальной системе базы
                # крана. Затем их координаты переводятся в общую ГСК.
    ...
    if cfg.use_rope_throwover:
        alpha_pen = rope_data[-2]["alpha_rope"]
        if abs(alpha_pen) > 90.0:
