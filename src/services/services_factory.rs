...
impl ServicesFactory {
    ...
    #[named]
    pub fn service(&self, kind: impl Into<String>, name: &str, conf: ConfTree, services: Arc<Services>, scheduler: Scheduler) -> Result<Arc<dyn Service>, Error> {
        let kind = &kind.into();
        match kind.as_ref() {
            Self::API_CLIENT => {
                let conf = ApiClientConf::new(&self.parent, conf);
                log::trace!("{}.run | Conf: {:#?}", self.dbg, conf);
                Ok(Arc::new(ApiClient::new(conf, services, scheduler.clone())))
            }
            Self::MULTI_QUEUE => {
                let conf = MultiQueueConf::new(&self.parent, conf);
                log::trace!("{}.run | Conf: {:#?}", self.dbg, conf);
                Ok(Arc::new(MultiQueue::new(conf, services, Some(scheduler.clone()))))
            }
            Self::PROFINET_CLIENT => {
                let conf = ProfinetClientConf::new(&self.parent, conf);
                log::trace!("{}.run | Conf: {:#?}", self.dbg, conf);
                Ok(Arc::new(ProfinetClient::new(conf, services, scheduler.clone())))
            }
            Self::TASK => {
                let conf = TaskConf::new(&self.parent, conf).map_err(|err| err_pass!(self.dbg, err))?;
                log::trace!("{}.run | Conf: {:#?}", self.dbg, conf);
                Ok(Arc::new(Task::new(conf, services.clone(), scheduler.clone())))
            }
            Self::TCP_CLIENT => {
                let conf = TcpClientConf::new(&self.parent, conf);
                log::trace!("{}.run | Conf: {:#?}", self.dbg, conf);
                Ok(Arc::new(TcpClient::new(conf, services.clone(), scheduler.clone())))
            }
            Self::TCP_SERVER => {
                let conf = TcpServerConf::new(&self.parent, conf);
                log::trace!("{}.run | Conf: {:#?}", self.dbg, conf);
                Ok(Arc::new(TcpServer::new(conf, services.clone(), scheduler.clone())))
            }
            Self::PRODUCER_SERVICE => {
                let conf = ProducerServiceConf::new(&self.parent, conf);
                log::trace!("{}.run | Conf: {:#?}", self.dbg, conf);
                Ok(Arc::new(ProducerService::new(conf, services.clone(), scheduler.clone())))
            }
            Self::CACHE_SERVICE => {
                let conf = CacheServiceConf::new(&self.parent, conf);
                log::trace!("{}.run | Conf: {:#?}", self.dbg, conf);
                Ok(Arc::new(CacheService::new(conf, services.clone(), scheduler.clone())))
            }
            Self::SLMP_CLIENT => {
