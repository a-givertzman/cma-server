...
use super::*;
    ...
    fn test_ne_both_new_true() {
        ...
        let mut ne = FnNe::new("test", vec![in1, in2]).unwrap();
        let res = ne.out().unwrap();
        assert!(matches!(res, Some(FnFlow::New(_))));
        assert_eq!(res.unwrap().into_value().to_bool().as_bool().value.0, true);
    }
    #[test]
    fn test_ne_both_old_optimization() {
        let ts = chrono::Utc::now();
    ...
    #[test]
    fn test_ne_one_new_triggers_recalculation() {
        let ts = chrono::Utc::now();
    ...
    #[test]
    fn test_ne_none_short_circuit_propagation() {
        let ts = chrono::Utc::now();
        let p1 = Point::Double(PointHlr::new(1, "p1", 3.0, Status::Ok, Cot::Inf, ts));
        let in1 = create_mock(FnFlow::New(p1));
