...
mod tests {
    ...
    #[test]
    fn test_lt_one_new_triggers_recalculation() {
        let ts = chrono::Utc::now();
    ...
    #[test]
    fn test_lt_none_short_circuit_propagation() {
        let ts = chrono::Utc::now();
        let p1 = Point::Double(PointHlr::new(1, "p1", 3.0, Status::Ok, Cot::Inf, ts));
        let in1 = create_mock(FnFlow::New(p1));
