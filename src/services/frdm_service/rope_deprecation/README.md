...
Сервис считает накопленный износ (deprecation rate) стального/текстового каната
...
4. Агрегирует пары `(slice_ix, deprecation)`, формирует `INSERT ... ON CONFLICT`
...
### Полный пример (актуальный формат)
...
| Параметр | Тип | Обяз. | Описание |
|----------|-----|-------|----------|
| `width` | Distance | **да** | Диаметр каната. Используется для валидации `segment`. |
...
| `segment` | Distance | **да** | Длина расчётного сегмента. Весь канат делится на сегменты; износ считается per-segment. Ограничение: `0.5·width ≤ segment ≤ 7·width`. Меньше сегмент — выше точность, больше вычислений. |
| `pos` | point real | **да** | Имя точки позиции каната (м). Значение из шины. |
| `load` | point real | **да** | Имя точки нагрузки (т). Значение из шины. |
...
| `scheme` | enum | **да** | Схема схода каната: `TopTop`, `TopBottom`, `BottomTop`, `BottomBottom`. |
| `bind` | enum | **да** | Привязка: `Drum` (барабан), `Fixed` (неподвижный вне стрел), `Boom N` (блок на N-ной стреле), `Hook` (подвес). |
| `deflector-angle` | Angle | нет | Угол перекидывания. Блок включается в работу только когда стрела проходит это положение. |
...
1. **Имена точек** в `pos`/`load`/`angle` должны точно совпадать с именами
   событий на шине MultiQueue. В тестовых конфигах используются короткие
   имена (`'Winch.Pos'`), в интеграционном — с путём (`'/App/VirtualIed/Winch.Pos'`).
   Несовпадение приводит к молчаливому игнорированию (warn в логах `Inputs`).

2. **`segment` и `width`** связаны валидацией: `0.5·width ≤ segment ≤ 7·width`.
   Выход за границы — panic при парсинге конфига.

3. **Накопление износа** в БД идёт через `ON CONFLICT DO UPDATE SET
   deprecation = deprecation + EXCLUDED.deprecation`. Тип колонки
   `deprecation` должен допускать суммирование (`numeric` предпочтительнее
   `double precision` для длительной эксплуатации).

4. **`wait-started`** у `rope-deprecation` не является обязательным полем
   конфига, но рекомендуется: он гарантирует, что рабочий поток дошёл до
