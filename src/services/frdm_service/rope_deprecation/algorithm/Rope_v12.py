...
def rope_parameters(block, X1, Y1, X2, Y2, D1, D2, k, j):
    """
    ...
    logging.debug(f"block[{block}] l_block: {l_block}")

    alpha_block = alpha_horiz(Y1, Y2, X1, X2)
    alpha_rope = alpha_block + j * math.degrees(math.asin((0.5 * (D1 + k * D2) / l_block) % 1))
...
def ensure_min_hook_length(booms, blocks: list[Block], rope_calc_params):
    """
    ...
    if x < 0:
        ...
        last_seg = rope_parameters(prev_idx, X1, Y1, X2, Y2, D1, D2, k, j)
...
if __name__ == "__main__":
    ...
    next(rows)
    tblock: list[Block] = [Block.empty(), Block.empty(), Block.empty(), Block.empty(), Block.empty(), Block.empty(), Block.empty(), Block.empty()]
    for row in rows:
        ...
        blocks = [
            Block(lF=Offset( 1830.0,  710.0), D=844.000, scheme=1, bind=BlockBindFixed()),
        ...
        for i, block in enumerate(blocks[:-1]):
            ...
            X2, Y2 = blocks[i].coord.x, blocks[i + 1].coord.y
            D1 = block.D
            D2 = blocks[i + 1].D
            scheme = block.scheme
            ...
            params = rope_parameters(i, X1, Y1, X2, Y2, D1, D2, k, j)
        ...
        if alpha_pen > 90.0:
            ...
            new_params = rope_parameters(4, X1, Y1, X2, Y2, D1, D2, k, j)
        ...
        logging.debug('-'*40)
        logging.debug(f"Step {step}, Итоги расчёта каната")
