...
if __name__ == "__main__":
    ...
    # -----------------------------    
    def build_support_points(rope_results, rope_data, block_results):
    ...
    def get_most_common_bend_coords(L_rope_vector, counts, most_common_val, rope_data):
        """
        далее нам нужно найти координаты точек каната, которые мы определили в предыдущем этапе
        Берём из рассчитанных итогов длину каната, которая уже лежит на барабане (L_winch, мм).
        Находим индексы точек каната, где число перегибов равно наиболее частому значению most_common_val.
        Переводим вектор длин каната L_rope_vector из метров в миллиметры, чтобы привести к тем же единицам, что и в rope_data (там всё в мм).
        """
        L_winch = rope_results["L_winch"]
        coords = []
        indices = np.where(counts == most_common_val)[0]

        # Переводим в мм
        L_rope_mm = L_rope_vector * 1000
    
        for idx in indices:
            L = L_rope_mm[idx]  # теперь в мм
            total_length = L_winch # длина каната на лебедке
            for r in rope_data:
                segment_length = r['l_rope']
                if total_length <= L <= total_length + segment_length:
                    s = (L - total_length) / segment_length
                    x = r['X1_block'] + s * (r['X2_block'] - r['X1_block'])
                    y = r['Y1_block'] + s * (r['Y2_block'] - r['Y1_block'])
                    coords.append((x, y))
                    break
                total_length += segment_length
    
        return np.array(indices, dtype=int), coords
...
# Строим координаты точек, с наиболее частым числом перегибов
indices, coords = get_most_common_bend_coords(L_rope_vector, counts, most, rope_data)
...
# Перегибы каната (most common bends)
xs, ys = zip(*coords)  # разделяем X и Y
plt.scatter(xs, ys, color='red', s=15, marker='o', label='Перегибы каната')
