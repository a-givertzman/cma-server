...
### Два размера сегмента

- `segment` - Малый сегмент для фотографирования и анализа, его размер определяет размер кадра камеры (длина каната в кадре). Допустимое значение в конфигурации 24..500 мм. Однако в БД сегментирование каната устроено иначе.
- `register_segment` - Размер сегмента для регистрации в БД. Любой сегмент каната, попавший в кадр камеры с округлением принадлежит условно "номеру метра" каната. Этот индекс и будет БД. Приведение индекса из "Малого" в "БД" выполняет функция `RopeDefectConf::scale_slice_to_db` в колбэке `FineScan`. Малый сегмент `ix` относится к тому регистрационному сегменту, на котором лежит большая его часть.
...
| Поле | Тип | Описание |
|------|-----|----------|
| `slice` | bigint | Номер сегмента каната для регистрации, не путать с номером сегмента для фотографирования. |
...
| `segment` | Distance | **да** | Длина расчётного сегмента каната для фотографирования и анализа. Весь канат делится на сегменты; кадр обрабатывается при прохождении камерой границы сегмента. Рекомендация: `segment = (0.80..0.90) * ширина кадра камеры` (вдоль каната). Допустимый диапазон: `24..500 mm`. |
| `register_segment` | Distance | **Auto** | Размер сегмента для регистрации дефектов в БД, Вычисляется автоматически от `segment`, для большенства случаев равен 1м. |
...
> **Auto** - Значение вычисляется автоматически, пользователь не задает.

#### Размеры сегментов каната
...
Крупный сегмент регистрации собирает несколько сегментов анализа, поэтому ошибка
синхронизации кадра и позиции каната (кадры и позиция приходят из разных мест и
могут запаздывать относительно друг друга) не уводит дефект в чужой сегмент.
Число регистрационных сегментов рассчитывается от полной длины каната из секции
`rope-deprecation` (`RopeDefectConf::db_slices`) и пишется в `frdm_settings`
как `winchN-defect_slices`.
...
5. **Хранилище изображений.** Изображения дефектов сохраняются в
   `assets/files/<имя-сервиса>/rope-defects/<slice>/*.jpg`, где `<slice>` —
   номер регистрационного сегмента (`register_segment`), совпадающий с
   `frdm_defect.slice`. Имя файла содержит дату, вид дефекта, timestamp
   и порядковый номер. Количество изображений на сегмент ограничено функцией
   БД `clean_frdm_defect_image` (10 штук).
