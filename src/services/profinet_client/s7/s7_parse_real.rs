...
mod s7_parse_real_test {
    ...
    fn to_point() {
        ...
        init_each();
        log::debug!("");
        let dbg = Dbg::own("S7ParseReal-to_point");
        log::debug!("\n{}", dbg);
        ...
        let mut parse = S7ParseReal::new(0, Name::new(&dbg, "S7ParseReal").join(),
            &PointConf {
                id: 0,
                name: Name::new(&dbg, "S7ParseReal").join(),
    ...
    fn add_raw() {
        ...
        init_each();
        log::debug!("");
        let dbg = Dbg::own("S7ParseReal-add_raw");
        log::debug!("\n{}", dbg);
        let test_duration = TestDuration::new(&dbg, Duration::from_secs(1));
        test_duration.run().unwrap();
        // ----------------------------------------------------------------
        ...
        let mut parse = S7ParseReal::new(
            0,
            Name::new(&dbg, "S7ParseReal").join(),
            &PointConf {
                id: 0,
                name: Name::new(&dbg, "S7ParseReal").join(),
                type_: PointType::Real,
