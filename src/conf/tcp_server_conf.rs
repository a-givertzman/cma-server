...
impl TcpServerConf {
    ...
    ///     send-to: MultiQueue.queue
    ///                     ...
    pub fn new(parent: impl Into<String>, conf: ConfTree) -> TcpServerConf {
        let me = conf.sufix_or(conf.name().unwrap());
        let dbg = format!("TcpServerConf({})", me);
    ...
    ///
    /// creates config from serde_yaml::Value of following format:
    pub(crate) fn from_yaml(parent: impl Into<String>, value: &serde_yaml::Value) -> TcpServerConf {
        match value.as_mapping().unwrap().into_iter().next() {
            Some((key, value)) => {
                Self::new(parent, ConfTree::new(key.as_str().unwrap(), value.clone()))
            }
            None => {
                panic!("TcpServerConf.from_yaml | Format error or empty conf: {:#?}", value)
            }
        }
    }
    ///
    ...
    #[allow(dead_code)]
    pub fn read(parent: impl Into<String>, path: &str) -> TcpServerConf {
        match fs::read_to_string(path) {
            Ok(yaml_string) => {
                match serde_yaml::from_str(&yaml_string) {
                    Ok(config) => {
                        TcpServerConf::from_yaml(parent, &config)
                    }
                    Err(err) => {
                        panic!("TcpServerConf.read | Error in config: {:?}\n\terror: {:?}", yaml_string, err)
                    }
                }
            }
            Err(err) => {
                panic!("TcpServerConf.read | File {} reading error: {:?}", path, err)
            }
        }
    }
}
