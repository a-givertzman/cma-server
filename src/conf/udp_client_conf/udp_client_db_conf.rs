...
//
// 
impl UdpClientDbConf {
    ///
    /// Creates [UdpClientDbConf] new instance
    pub fn new(parent: impl Into<String>, name: &str, conf: ConfTree) -> Self {
        log::trace!("UdpClientDbConf.new | conf: {:?}", conf);
        let self_conf = conf.clone();
        let self_id = format!("UdpClientDbConf({})", self_conf.key);
