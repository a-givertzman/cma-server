...
mod cma_recorder {
    ...
    fn operating_cycle_metric() {
        ...
        //  step    nape                                input                    Pp Cycle   target_thrh             target_smooth
            ("00.0",    format!("/{}/Load.Nom", dbg),   Value::Real(  150.00),     0,       00.0000,                0.0f32),
            ...
            ("00.4",    format!("/{}/Load", dbg),       Value::Real(  0.00),       0,       00.0000,                0.0),
        ...
        let total_count_load = test_data.iter().filter(|(_, name, _, _, _, _)| *name == format!("/{}/Load", dbg)).count();
