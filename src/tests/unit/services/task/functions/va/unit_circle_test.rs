...
mod unit_circle {
    ...
    fn next_8() {
        ...
        for (angle, complex) in test_data {
            ...
            let time = unit_circle.next();
            let (angle, complex) = unit_circle.at(time);
    ...
    ///
    /// Testing UnutCycle.at()   0, 30, 60, 90, 120, 150, 180, 210, 240, 270, 300, 330, 360 grad
    #[test]
    fn at_12() {
        DebugSession::init(LogLevel::Debug, Backtrace::Short);
        init_once();
        ...
        for (angle, complex) in test_data {
            ...
            let time = angle / ((freq as f64) * UnitCircle::PI2);
            let t = unit_circle.next();
            let (angle, complex) = unit_circle.at(time);
            log::debug!("time: {}, sec,   next.time: {}", time, t);
            let k = (target_angle / (2.0 * PI)).trunc();
            let result = angle;// + 2.0 * PI * k;
            let target = target_angle;
            assert!(result.aprox_eq(target, 8), "time, sec: {}  angle {} ({}) \nresult: {:?}\ntarget: {:?}", time, angle, target_angle_grad, result, target);
