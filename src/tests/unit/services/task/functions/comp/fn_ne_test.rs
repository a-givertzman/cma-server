...
use crate::{
    domain::FnInOutRef, 
    services::task::{EvalCycle, EvalCycleRef, FnInput, FnNe, FnOut},
};
///
///
...
fn test_bool() {
    ...
    for (step, value1, value2) in test_data {
        ...
        let result = fn_ne.out().unwrap().unwrap().into_value().as_bool().value.0;
