...
use crate::{services::{Blocks, Booms, CraneConf, FrdmServiceConf, Inputs}, tests::unit::services::frdm_service::{CsvRecord, CsvTable}};
...
fn new() {
            ...
            for row in csv {
                test_data.push((
                    ...
                    [
                        ...
                        (row.cell::<f64>("x1").unwrap(),        row.cell::<f64>("y1").unwrap(),     row.cell::<f64>("rope_alpha1").unwrap()),
                        (row.cell::<f64>("x2").unwrap(),        row.cell::<f64>("y2").unwrap(),     row.cell::<f64>("rope_alpha2").unwrap()),
                        (row.cell::<f64>("x3").unwrap(),        row.cell::<f64>("y3").unwrap(),     row.cell::<f64>("rope_alpha3").unwrap()),
                        (row.cell::<f64>("x4").unwrap(),        row.cell::<f64>("y4").unwrap(),     row.cell::<f64>("rope_alpha4").unwrap()),
                        (row.cell::<f64>("x5").unwrap(),        row.cell::<f64>("y5").unwrap(),     row.cell::<f64>("rope_alpha5").unwrap()),
                        (row.cell::<f64>("x6").unwrap(),        row.cell::<f64>("y6").unwrap(),     row.cell::<f64>("rope_alpha6").unwrap()),
                        (row.cell::<f64>("x_hook").unwrap(),    row.cell::<f64>("y_hook").unwrap(), f64::NAN),
                    ],
                ));
            }
    ...
    for (path, conf, test_data) in test_data {
        ...
        for (step, events, target) in test_data {
            ...
            log::debug!("step {step}  result block pos: \n\t{:?}", result.iter().map(|b| format!("{:.3}, {:.3}", b.pos.x, b.pos.y)).collect::<Vec<_>>());
            log::debug!("step {step}  target block pos: \n\t{:?}", target.iter().map(|(x, y, _)| format!("{:.3}, {:.3}", x, y)).collect::<Vec<_>>());
            if target.iter().zip(&result).any(|((x, y, _), b)| (x - b.pos.x).abs() > 1.0 || (y - b.pos.y).abs() > 1.0) {
                // log::debug!("step {step}  pos delta       : {:?}", target.iter().zip(&result).map(|((x, y, _), b)| format!("{:.3}, {:.3}", x - b.pos.x, y - b.pos.y)).collect::<Vec<_>>()));
                log.push(format!("step {step}  pos delta       : {:?}", target.iter().zip(&result).map(|((x, y, _), b)| format!("{:.3}, {:.3}", x - b.pos.x, y - b.pos.y)).collect::<Vec<_>>()));
            }
            log::debug!("step {step}  target rope alpha    : \n\t{:?}", target.iter().map(|(_, _, a)| format!("{:.3}", a)).collect::<Vec<_>>());
            log::debug!("step {step}  result rope alpha fwd: \n\t{:?}", result.iter().map(|b| format!("{:.3}", b.rope_alpha_fwd)).collect::<Vec<_>>());
